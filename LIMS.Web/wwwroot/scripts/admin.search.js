...
Admin.Search = (function () {
    var itemTemplate = function (data) {
        var path = function () {
            if (data.LIMSParent) {
                return $("<p/>").text([data.LIMSParent, data.parent].join(" > ")).html();
            }
            else {
                return data.parent;
            }
                ...
                case 1:
                    {
                        ...
                        map[href] = { title: title, link: href, parent: null, LIMSParent: null };
                        break;
                    }
                // items in level two, these items have parent but have not LIMS parent
                case 3:
                    {
                        href = $(parents).eq(0).find("a").attr("href");
                        title = $(parents).eq(0).find("a").find("span").html();
                        parent = $(parents).eq(2).find("a").find("span").html();
                        map[href] = { title: title, link: href, parent: parent, LIMSParent: null };
                        break;
                    }
                // items in level three, these items have both parent and LIMS parent
                case 5:
                    {
                        href = $(parents).eq(0).find("a").attr("href");
                        title = $(parents).eq(0).find("a").find("span").html();
                        parent = $(parents).eq(2).find("a").find("span").html();
                        LIMSParent = $(parents).eq(4).find("a").find("span").html();
                        map[href] = { title: title, link: href, parent: parent, LIMSParent: LIMSParent };
                        break;
                    }
                default: break;
            }
