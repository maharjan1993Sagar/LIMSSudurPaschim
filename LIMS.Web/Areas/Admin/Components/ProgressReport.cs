    ...
    public class ProgressReport : BaseViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(string fiscalyear, string month,string xetra)
        {
            try
            {
                ...
                //    xetra = "";
                //}
                var pragatis = await _monthlyProgressService.GetFilteredMonthlyPragati(createdBy, fiscalyear, "", "", "", "", xetra);

                var allBudget = await _budgetService.GetMainKharakram(createdBy, fiscalyear);

                var budget = new List<PujigatKharchaKharakram>();
                ...
                {
                    var lstprogress = pragatis.Where(m => m.PujigatKharchaId == item.Id);
                    ...
                    for (int j = 0; j <= serialNo; j++)
                    {
                        ...
                        if (lstMonthProgress.Count() > 0)
                        {
                            var objReportDatas = lstMonthProgress.Select(m => new ProgressRowData {
                                BudgetTitle = m.pujigatKharchaKharakram.Program,
                                Anugaman = m.Anugaman,
            ...
            {
                var objModel = new ProgressReportModel();
                objModel.Address = ex.Message.ToString();
                return View(objModel.Address);
            }
        }
    }
}
