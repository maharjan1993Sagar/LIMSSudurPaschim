...
{
    public class MeatProcessingController : BaseAdminController
    {
        private readonly IOtherOrganizationService _organizationService;
        private readonly IMeatProcessingIndustryService _meatProcessingIndustryService;
        ...
        public MeatProcessingController(IOtherOrganizationService organizationService,
            IMeatProcessingIndustryService meatProcessingIndustryService,
            ...
            )
        {
            ...
            _meatProcessingIndustryService = meatProcessingIndustryService;
        ...
        public async Task<IActionResult> List(DataSourceRequest command,string Keyword)
        {
            ...
            {
                var hatchery = await _meatProcessingIndustryService.GetMeatProcessingIndustryByType(createdby, "Meat processing industry",Keyword, command.Page - 1, command.PageSize);
            ...
            else if (roles.Contains(RoleHelper.VhlsecAdmin) || roles.Contains(RoleHelper.VhlsecUser))
            {
                ...
                var hatchery = await _meatProcessingIndustryService.GetMeatProcessingIndustryByType(userids, "Meat processing industry", Keyword, command.Page - 1, command.PageSize);
                var gridModel = new DataSourceResult {
                    Data = hatchery,
                    ExtraData = Keyword,
                    Total = hatchery.TotalCount
        ...
        public async Task<ActionResult> Create()
        {
            ...
            var organization = await _organizationService.GetOtherOrganizationByType(createdby, "Meat processing industry");
            var fiscalyear = new SelectList(await _fiscalYearService.GetFiscalYear(), "Id", "NepaliFiscalYear").ToList();
            fiscalyear.Insert(0, new SelectListItem(_localizationService.GetResource("Admin.Common.Select"), ""));
            ViewBag.FiscalYearId = fiscalyear;
            var feedIndustry = new MeatProcessingModel();
            feedIndustry.Organization = organization;
            return View(feedIndustry);
        }
        [HttpPost]
        public async Task<ActionResult> Create(MeatProcessingModel model, IFormCollection form)
        {
            var dailyProcessingCapacity = form["DailyProcessingCapacity"].ToList();
            var chickenSusage = form["ChickenSusage"].ToList();
            var porkSusage = form["PorkSusage"].ToList();
            var sukutiBuff = form["SukutiBuff"].ToList();
            var fish = form["Fish"].ToList();
            var buffSusage = form["BuffSusage"].ToList();
            var other = form["Others"].ToList();
            var ProcessedMeat = form["ProcessedMeat"].ToList();
            var market = form["Market"].ToList();
            var FemaleManpower = form["FemaleManpower"].ToList();
            var maleManpower = form["MaleManpower"].ToList();
            var yearlyBusiness = form["YearlyBusiness"].ToList();
            ...
            var feedIndustries = new List<MeatProcesssingIndustries>();
            for (int i = 0; i < organizationIds.Count(); i++)
            {
                if (string.IsNullOrEmpty(organizationIds[i]))
                    continue;

                var otherOrganization = new MeatProcesssingIndustries {
                    ...
                    DailyProcessingCapacity = dailyProcessingCapacity[i],
                    ChickenSusage = chickenSusage[i],
                    PorkSusage = porkSusage[i],
                    SukutiBuff = sukutiBuff[i],
                    BuffSusage = buffSusage[i],
                    Fish = fish[i],
                    ProcessedMeat = ProcessedMeat[i],
                    Others = other[i],
                    Market = market[i],
                    FemaleManpower=FemaleManpower[i],
                    MaleManpower=maleManpower[i],
                    YearlyBusiness = yearlyBusiness[i],
                    CreatedBy = createdby,
            ...
            await _meatProcessingIndustryService.InsertMeatProcessingIndustryList(feedIndustries);
