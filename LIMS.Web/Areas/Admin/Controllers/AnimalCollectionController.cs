...
{
    public class AnimalCollectionController:BaseAdminController
    {
        ...
        public AnimalCollectionController(IOtherOrganizationService organizationService,
        ...
        public async Task<IActionResult> List(DataSourceRequest command, string Keyword = "")
        {
            ...
            //{
                var hatchery = await _otherOrganizationDetailsService.GetOtherFilteredOrganization(createdby, "Animal collection", Keyword, command.Page - 1, command.PageSize);
            ...
            //    var hatchery = await _otherOrganizationDetailsService.GetOtherFilteredOrganization(userids, "Animal collection", Keyword, command.Page - 1, command.PageSize);
        ...
        {
            var placeForCOllection = form["PlaceForCOllection"].ToList();
            var animalType = form["AnimalType"].ToList();
            var yearlySoldAnimals = form["TotalNoOfSoldAnimals"].ToList();
            var totalBusiness = form["TotalBusiness"].ToList();
            ...
            for (int i = 0; i < organizationIds.Count(); i++)
            {
                ...
                var otherOrganization = new OtherOrganizationDetails {
                    ...
                    TotalNoOfSoldAnimals = yearlySoldAnimals[i],
                    TotalBusiness = totalBusiness[i],
                    CreatedBy = createdby,
                    FiscalYear = await _fiscalYearService.GetFiscalYearById(model.FiscalYearId),
                    FiscalYearId=model.FiscalYearId,
                    PlaceForCOllection=placeForCOllection[i]
