...
{
    public class FeedAndFodderResearchCenterController : BaseAdminController
    {
        private readonly IOtherOrganizationService _organizationService;
        private readonly IFeedFodderResearchCenterService _feedFodderResearchCenterService;
        ...
        public FeedAndFodderResearchCenterController(IOtherOrganizationService organizationService,
             IFeedFodderResearchCenterService feedFodderResearchCenterService,
            ...
            )
        {
            ...
            _feedFodderResearchCenterService = feedFodderResearchCenterService;
        ...
        public async Task<IActionResult> List(DataSourceRequest command,string Keyword)
        {
            ...
            //{
                var hatchery = await _feedFodderResearchCenterService.GetFeedFodderResearchCenter(createdby,Keyword, command.Page - 1, command.PageSize);
            ...
            //    var hatchery = await _feedFodderResearchCenterService.GetFeedFodderResearchCenter(userids,Keyword, command.Page - 1, command.PageSize);
            //    var gridModel = new DataSourceResult {
            //        Data = hatchery,
            //        ExtraData = Keyword,
            //        Total = hatchery.TotalCount
        ...
        public async Task<ActionResult> Create()
        {
            ...
            var organization = await _organizationService.GetOtherOrganizationByType("", "Feed/fooder Resource Center");
        ...
        [HttpPost]
        public async Task<ActionResult> Create(FeedFodderResearchCenterModel model, IFormCollection form)
        {
            var feedFodderFound = form["FeedFodderFound"].ToList();
            var areaForFeedAndFodder = form["AreaForFeedAndFodder"].ToList();
            var quantityForSeeds = form["QuantityForSeeds"].ToList();
            var womenManpower = form["WomenManpower"].ToList();
            ...
            //    createdby = admin.Id;
            //}
            var feedIndustries = new List<FeedFodderResearchCenter>();
            for (int i = 0; i < organizationIds.Count(); i++)
            {
                if (string.IsNullOrEmpty(organizationIds[i]))
                    continue;

                var otherOrganization = new FeedFodderResearchCenter {
                    ...
                    FeedFodderFound = feedFodderFound[i],
                    AreaForFeedAndFodder = areaForFeedAndFodder[i],
                    QuantityForSeeds = quantityForSeeds[i],
                    WomenManpower = womenManpower[i],
            ...
            await _feedFodderResearchCenterService.InsertFeedFodderResearchCenterList(feedIndustries);
