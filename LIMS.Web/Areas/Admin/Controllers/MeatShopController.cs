...
{
    public class MeatShopController:BaseAdminController
    {
        private readonly IOtherOrganizationService _organizationService;
        private readonly IMeatShopService _meatShopService;
        ...
        public MeatShopController(IOtherOrganizationService organizationService,
            IMeatShopService meatShopService,
            ...
            )
        {
            ...
            _meatShopService = meatShopService;
        ...
        public async Task<IActionResult> List(DataSourceRequest command,string Keyword)
        {
            ...
            {
                var hatchery = await _meatShopService.GetMeatShop(createdby,Keyword, command.Page - 1, command.PageSize);
            ...
            else if (roles.Contains(RoleHelper.VhlsecAdmin) || roles.Contains(RoleHelper.VhlsecUser))
            {
                ...
                var hatchery = await _meatShopService.GetMeatShop(userids, Keyword, command.Page - 1, command.PageSize);
                var gridModel = new DataSourceResult {
                    Data = hatchery,
                    ExtraData = Keyword,
                    Total = hatchery.TotalCount
        ...
        public async Task<ActionResult> Create()
        {
            ...
            var feedIndustry = new MeatShopModel();
        ...
        {
            var buff = form["Buff"].ToList();
            var chicken = form["Chicken"].ToList();
            var mutton = form["Mutton"].ToList();
            var pork = form["Pork"].ToList();
            ...
            await _meatShopService.InsertMeatShopList(feedIndustries);
