...
namespace LIMS.Web.Areas.Admin.Controllers
{
    public class StaticVaccinationController : BaseAdminController
    {
        ...
        public async Task<IActionResult> List(DataSourceRequest command, string Fiscalyear, string Month, string SpeciesId )
        {
           ...
           // var fiscalyear = await _fiscalYearService.GetCurrentFiscalYear();
            ...
            if (Fiscalyear != null)
            {
                ...
                service = await _serviceData.GetFilteredByFarmListModel(createdby, "Vaccination", SpeciesId, "", Fiscalyear,Month, command.Page - 1, command.PageSize);

            }
            else
            {
                service = await _serviceData.GetFilteredByFarmListModel(createdby, "Vaccination", SpeciesId, "", CurrentFiscalYear,Month, command.Page - 1, command.PageSize);
        ...
        public async Task<ActionResult> Create()
        {
            ...
            model.Species = aiSpecies.ToList();
            return View(model);
        }
        ...
        public async Task<IActionResult> GetServiceData(string fiscalyearId, string month, string district, string locallevel, string vaccineName="")
        {
            ...
            var servicesData = await _serviceData.GetFilteredService(fiscalyearId, month, "Vaccination", createdby, district, locallevel,vaccineName,"","");
