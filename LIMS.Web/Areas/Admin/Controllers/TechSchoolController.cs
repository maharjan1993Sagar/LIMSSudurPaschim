...
{
    public class TechSchoolController:BaseAdminController
    {
        private readonly IOtherOrganizationService _organizationService;
        private readonly ITechSchoolService _nGOService;
        ...
        public TechSchoolController(IOtherOrganizationService organizationService,
            ITechSchoolService nGOService,
        ...
        public async Task<IActionResult> List(DataSourceRequest command,string Keyword)
        {
            ...
            //{
                var hatchery = await _nGOService.GetTechSchool(createdby,Keyword, command.Page - 1, command.PageSize);
            ...
            //    var hatchery = await _nGOService.GetTechSchool(userids, Keyword, command.Page - 1, command.PageSize);
            //    var gridModel = new DataSourceResult {
            //        Data = hatchery,
            //        ExtraData = Keyword,
            //        Total = hatchery.TotalCount
        ...
        public async Task<ActionResult> Create()
        {
            ...
            var organization = await _organizationService.GetOtherOrganizationByType(createdby, "Tech school");
            var fiscalyear = new SelectList(await _fiscalYearService.GetFiscalYear(), "Id", "NepaliFiscalYear").ToList();
            fiscalyear.Insert(0, new SelectListItem(_localizationService.GetResource("Admin.Common.Select"), ""));
            ViewBag.FiscalYearId = fiscalyear;
            var otherOrg = new TechSchoolModel();
            otherOrg.Organization = organization;
            return View(otherOrg);
        }
        [HttpPost]
        public async Task<ActionResult> Create(TechSchoolModel model, IFormCollection form)
        {
            var subjectToBeTaught = form["SubjectToBeTaught"].ToList();
            var duration = form["Duration"].ToList();
            var coordinatingBody = form["CoordinatingBody"].ToList();
            ...
            //    createdby = admin.Id;
            //}
            var otherOrganizationList = new List<TechSchool>();
            for (int i = 0; i < organizationIds.Count(); i++)
            {
                if (string.IsNullOrEmpty(organizationIds[i]))
                    continue;

                var otherOrganization = new TechSchool {
                    ...
                    SubjectToBeTaught = subjectToBeTaught[i],
                    Duration = duration[i],
                    CoordinatingBody = coordinatingBody[i],
                    Remarks = remarks[i],
            ...
            await _nGOService.InsertTechSchoolList(otherOrganizationList);
