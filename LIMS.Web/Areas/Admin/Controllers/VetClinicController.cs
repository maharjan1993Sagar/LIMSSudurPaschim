...
{
    public class VetClinicController:BaseAdminController
    {
        private readonly IOtherOrganizationService _organizationService;
        private readonly IVetClinicService _vetClinicService;
        ...
        public VetClinicController(IOtherOrganizationService organizationService,
            IVetClinicService vetClinicService,
            ...
            )
        {
            ...
            _vetClinicService = vetClinicService;
        ...
        public async Task<IActionResult> List(DataSourceRequest command,string Keyword)
        {
            ...
            //{
                var hatchery = await _vetClinicService.GetVetClinic(createdby, Keyword, command.Page - 1, command.PageSize);
            ...
            //    var hatchery = await _vetClinicService.GetVetClinic(userids, Keyword, command.Page - 1, command.PageSize);
            //    var gridModel = new DataSourceResult {
            //        Data = hatchery,
            //        ExtraData = Keyword,
            //        Total = hatchery.TotalCount
        ...
        public async Task<ActionResult> Create()
        {
            ...
            var organization = await _organizationService.GetOtherOrganizationByType(createdby, "Vet clinic");
            ...
            var otherOrg = new VetClinicModel();
            otherOrg.Organization = organization;
            return View(otherOrg);
        }
        [HttpPost]
        public async Task<ActionResult> Create(VetClinicModel model, IFormCollection form)
        {
            ...
            //    createdby = admin.Id;
            //}
            var otherOrganizationList = new List<VetClinic>();
            for (int i = 0; i < organizationIds.Count(); i++)
            {
                if (string.IsNullOrEmpty(organizationIds[i]))
                    continue;

                var otherOrganization = new VetClinic {
            ...
            await _vetClinicService.InsertVetClinicList(otherOrganizationList);
