...
{
    public class DiaryIndustryController :BaseAdminController
    {
        ...
        public DiaryIndustryController(IOtherOrganizationService organizationService,
        ...
        public async Task<IActionResult> List(DataSourceRequest command,string Keyword="")
        {
            ...
            {
                var hatchery = await _diaryIndustryService.GetDiaryIndustryAndShopByFiscalyear(createdby, "Diary industry", Keyword, command.Page - 1, command.PageSize);
            ...
            else if (roles.Contains(RoleHelper.VhlsecAdmin) || roles.Contains(RoleHelper.VhlsecUser))
            {
                ...
                var hatchery = await _diaryIndustryService.GetDiaryIndustryAndShopByFiscalyear(userids, "Diary industry", Keyword, command.Page - 1, command.PageSize);
                var gridModel = new DataSourceResult {
                    Data = hatchery,
                    ExtraData = Keyword,
                    Total = hatchery.TotalCount
        ...
        public async Task<ActionResult> Create(DiaryIndustryAndShopModel model, IFormCollection form)
        {
            ...
            var femaleMember = form["FemaleManPower"].ToList();
            ...
            var dailyMilkConsumption = form["TotalMilkConsumption"].ToList();
            string createdby = null;
            List<string> roles = _context.CurrentCustomer.CustomerRoles.Select(x => x.Name).ToList();
            if (roles.Contains(RoleHelper.LssAdmin) || roles.Contains(RoleHelper.VhlsecAdmin) || roles.Contains(RoleHelper.DolfdAdmin))
            {
            ...
            for (int i = 0; i < organizationIds.Count(); i++)
            {
                ...
                var diary = new DiaryIndustryAndShop {
                    ...
                    Others = others[i],
                    Churpi = churpi[i],
                    TotalMilkConsumption = dailyMilkConsumption[i],
                    MaleManPower=malemanpower[i],
                    FemaleManPower=femaleMember[i],
                    CreatedBy = createdby,
