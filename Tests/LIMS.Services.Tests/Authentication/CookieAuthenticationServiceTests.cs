...
namespace LIMS.Services.Tests.Authentication
{
    [TestClass()]
    public class CookieAuthenticationServiceTests
    {
        ...
        public void Init()
        {
            ...
            _config = new LIMSConfig();
            _config.CookieClaimsIssuer = "LIMS";
            _config.CookiePrefix = ".LIMS.";
        ...
        public async Task GetAuthenticatedCustomer_UsernameEnableRegisterd_ReturnCustomer()
        {
            ...
            var cliaim = new Claim(ClaimTypes.Name, "Johny","", "LIMS");
            IList <Claim> claims = new List<Claim>
            {
                 new Claim(ClaimTypes.Name,ClaimTypes.Name,"", "LIMS")
              };
            var principals = new ClaimsPrincipal(new ClaimsIdentity(claims, LIMSCookieAuthenticationDefaults.AuthenticationScheme));
            _authServiceMock.Setup(c => c.AuthenticateAsync(It.IsAny<HttpContext>(), It.IsAny<string>()))
