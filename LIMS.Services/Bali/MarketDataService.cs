using LIMS.Domain;
using LIMS.Domain.Bali;
using LIMS.Domain.Data;
using LIMS.Services.Events;
using MediatR;
using MongoDB.Driver.Linq;
using System;
using System.Collections.Generic;
using System.Text;
using System.Threading.Tasks;

namespace LIMS.Services.Bali
{
    public class MarketDataService:IMarketDataService
    {
        private readonly IRepository<MarketData> _marketDataRepository;
        private readonly IMediator _mediator;
        public MarketDataService(IRepository<MarketData> marketDataRepository, IMediator mediator)
        {
            _marketDataRepository = marketDataRepository;
            _mediator = mediator;
        }
        public async Task DeletemarketData(MarketData marketData)
        {
            if (marketData == null)
                throw new ArgumentNullException("MarketData");

            await _marketDataRepository.DeleteAsync(marketData);

            //event notification
            await _mediator.EntityDeleted(marketData);
        }

        public async Task<IPagedList<MarketData>> GetmarketData(string createdby, int pageIndex = 0, int pageSize = int.MaxValue, string fiscalyear = "")
        {
            var query = _marketDataRepository.Table;
            query = query.Where(m => m.CreatedBy == createdby);
            if (!string.IsNullOrEmpty(fiscalyear))
            {
                query = query.Where(
                  m => m.FiscalYear.Id == fiscalyear
                );
            }

            return await PagedList<MarketData>.Create(query, pageIndex, pageSize);
        }

        public async Task<IPagedList<MarketData>> GetmarketData(List<string> createdby, int pageIndex = 0, int pageSize = int.MaxValue, string fiscalyear = "")
        {
            var query = _marketDataRepository.Table;
            query = query.Where(m => createdby.Contains(m.CreatedBy));
            if (!string.IsNullOrEmpty(fiscalyear))
            {
                query = query.Where(
                  m => m.FiscalYear.Id == fiscalyear
                );
            }

            return await PagedList<MarketData>.Create(query, pageIndex, pageSize);
        }

        public Task<MarketData> GetmarketDataById(string id)
        {
            return _marketDataRepository.GetByIdAsync(id);
        }

        public async Task InsertmarketData(MarketData marketData)
        {
            if (marketData == null)
                throw new ArgumentNullException("Livestock");

            await _marketDataRepository.InsertAsync(marketData);

            //event notification
            await _mediator.EntityInserted(marketData);
        }

        public Task InsertmarketDataList(List<MarketData> marketDatas)
        {
            throw new NotImplementedException();
        }

        public async Task UpdatemarketData(MarketData marketData)
        {
            if (marketData == null)
                throw new ArgumentNullException("baliregister");

            await _marketDataRepository.UpdateAsync(marketData);

            //event notification
            await _mediator.EntityUpdated(marketData);
        }

        public Task UpdatemarketDataList(List<MarketData> marketDatas)
        {
            throw new NotImplementedException();
        }
    }
}
