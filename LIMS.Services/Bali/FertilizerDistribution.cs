...
namespace LIMS.Services.Bali
{
    public class FertilizerDistributionService:IFertilizerDistributionService
    {
        ...
        public async Task<IPagedList<FertilizerDistribution>> GetFertilizerDistribution(string createdby, string fiscalYear = "", string district = "", string locallevel = "", int pageIndex = 0, int pageSize = int.MaxValue, string fiscalyear = "")
        {
            var query = _FertilizerDistributionRepository.Table;
            if(!String.IsNullOrEmpty(createdby))
            {
