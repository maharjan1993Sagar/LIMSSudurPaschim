...
namespace LIMS.Services.Customers
{
    public static class CustomerExtensions
    {
        ...
        public static string[] GetCustomerRoleName(this Customer customer, bool showHidden = false)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");

            var customerRolesName = customer.CustomerRoles
               .Where(cr => showHidden || cr.Active)
               .Select(cr => cr.Name)
               .ToArray();

            return customerRolesName;
        }
