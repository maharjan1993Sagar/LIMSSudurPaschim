...
namespace LIMS.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual async Task<CustomerRegistrationResult> RegisterCustomer(CustomerRegistrationRequest request)
        {
           ...
           // await _mediator.CustomerRegistrationEvent(result, request);
            ...
            if (registeredRole == null)
                throw new LIMSException("'Registered' role could not be loaded");
            ...
            //var customRole = await _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.ForumModerators);
            //request.Customer.CustomerRoles.Remove(customRole);
        ...
        public virtual async Task SetEmail(Customer customer, string newEmail)
        {
            ...
            if (newEmail == null)
                throw new LIMSException("Email cannot be null");

            newEmail = newEmail.Trim();
            string oldEmail = customer.Email;

            if (!CommonHelper.IsValidEmail(newEmail))
                throw new LIMSException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new LIMSException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));

            var customer2 = await _customerService.GetCustomerByEmail(newEmail);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new LIMSException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
        ...
        public virtual async Task SetUsername(Customer customer, string newUsername)
        {
            ...
            if (!_customerSettings.UsernamesEnabled)
                throw new LIMSException("Usernames are disabled");

            if (!_customerSettings.AllowUsersToChangeUsernames)
                throw new LIMSException("Changing usernames is not allowed");

            newUsername = newUsername.Trim();

            if (newUsername.Length > 100)
                throw new LIMSException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = await _customerService.GetCustomerByUsername(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new LIMSException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
