...
namespace LIMS.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        /// {0} : system name
        /// </remarks>
        private const string CUSTOMERROLES_BY_SYSTEMNAME_KEY = "LIMS.customerrole.systemname-{0}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string CUSTOMERROLES_PATTERN_KEY = "LIMS.customerrole.";
        private const string CUSTOMERROLESPRODUCTS_PATTERN_KEY = "LIMS.product.cr";
        ...
        /// {0} : customer role Id?
        /// </remarks>
        private const string CUSTOMERROLESPRODUCTS_ROLE_KEY = "LIMS.customerroleproducts.role-{0}";
        ...
        public virtual async Task<IPagedList<Customer>> GetAllCustomers(DateTime? createdFromUtc = null,
            DateTime? createdToUtc = null, string affiliateId = "", string vendorId = "", string storeId = "", string ownerId = "", string createdBy = "",
            string[] customerRoleIds = null, string[] customerTagIds = null, string email = null, string username = null,
            string firstName = null, string lastName = null,
            string company = null, string phone = null, string zipPostalCode = null,
            bool loadOnlyWithShoppingCart = false, int pageIndex = 0, int pageSize = 2147483647)
        {
        ...
        public List<Customer> GetCustomerByLssId(List<string> Lssid, string vhlsecid)
        {
            var query = _customerRepository.Table;


            if (Lssid != null && Lssid.Count > 0)
                query = query.Where(c => c.EntityId == vhlsecid || Lssid.Contains(c.EntityId));
            else
                query = query.Where(c => c.EntityId == vhlsecid);

            return query.ToList();
        }
        public List<Customer> GetNlboUsers()
        {
            var query = _customerRepository.Table;


                query = query.Where(c => c.EntityId == "Nlbo");

            return query.ToList();
        }


        public List<Customer> GetCustomerByLssId(List<string> lssid, List<string> vhlsecId, string dolfdId)
        {
            var query = _customerRepository.Table;
            query = query.Where(c => lssid.Contains(c.EntityId) || vhlsecId.Contains(c.EntityId) || c.EntityId == dolfdId);
            return query.ToList();
        }
        ...
        public virtual async Task DeleteCustomer(Customer customer, bool hard = false)
        {
            ...
            if (customer.IsSystemAccount)
                throw new LIMSException(string.Format("System customer account ({0}) could not be deleted", customer.SystemName));
        ...
        public virtual async Task<Customer> InsertGuestCustomer(Store store, string urlreferrer = "")
        {
            ...
            if (guestRole == null)
                throw new LIMSException("'Guests' role could not be loaded");
        ...
        public virtual async Task UpdateCustomerinAdminPanel(Customer customer)
        {
            ...
            var update = Builders<Customer>.Update
                ...
                .Set(x => x.District, customer.District)
                ...
                .Set(x => x.Active, customer.Active)
                .Set(x => x.OrgAddress, customer.OrgAddress)
                .Set(x => x.OrgName, customer.OrgName)
                .Set(x => x.NameEnglish, customer.NameEnglish)
                .Set(x => x.NameNepali, customer.NameNepali)
                .Set(x => x.Position, customer.Position)
                .Set(x => x.EntityId, customer.EntityId)

                .Set(x => x.Province, customer.Province);
        ...
        public virtual async Task DeleteCustomerRole(CustomerRole customerRole)
        {
            ...
            if (customerRole.IsSystemRole)
                throw new LIMSException("System role could not be deleted");
