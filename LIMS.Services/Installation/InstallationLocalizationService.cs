...
namespace LIMS.Services.Installation
{
    ...
    public partial class InstallationLocalizationService : IInstallationLocalizationService
    {
        ...
        /// </summary>
        private const string LANGUAGE_COOKIE_NAME = ".LIMS.installation.lang";
            ...
            foreach (var filePath in System.IO.Directory.EnumerateFiles(CommonHelper.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
            {
                ...
                foreach (XmlNode resNode in resources)
                {
                    ...
                    if (resNameAttribute == null)
                        throw new LIMSException("All installation resources must have an attribute Name=\"Value\".");
                    var resourceName = resNameAttribute.Value.Trim();
                    if (string.IsNullOrEmpty(resourceName))
                        throw new LIMSException("All installation resource attributes 'Name' must have a value.'");

                    if (resValueNode == null)
                        throw new LIMSException("All installation resources must have an element \"Value\".");
