            ...
            if (bundleFiles.Value)
            {
                ...
                if (partsToBundle.Any())
                {
                    ...
                    foreach (var item in partsToBundle)
                    {
                        ...
                        if (LIMS.Core.OperatingSystem.IsWindows())
                    ...
                    //save
                    string configFilePath = _hostingEnvironment.ContentRootPath + (LIMS.Core.OperatingSystem.IsWindows() ? "\\" : "/") + outputFileName + ".json";
                    ...
                    var fileNames = filePaths.Select(x => x.Substring(x.LastIndexOf((LIMS.Core.OperatingSystem.IsWindows() ? "\\" : "/")) + 1));
                    ...
                    //render
                    if (File.Exists(bundleDirectory + (LIMS.Core.OperatingSystem.IsWindows() ? "\\" : "/") + outputFileName + ".min.js"))
            ...
            if (bundleFiles.Value)
            {
                ...
                if (partsToBundle.Any())
                {
                    ...
                    foreach (var item in partsToBundle)
                    {
                        ...
                        src = urlHelper.Content(src);
                        //check whether this file exists 
                        var srcPath = LIMS.Core.OperatingSystem.IsWindows() ?
                    ...
                    //render
                    if (File.Exists(bundleDirectory + (LIMS.Core.OperatingSystem.IsWindows() ? "\\" : "/") + outputFileName + ".min.css"))
