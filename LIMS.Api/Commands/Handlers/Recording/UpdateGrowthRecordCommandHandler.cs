...
{
    public class UpdateGrowthRecordCommandHandler : IRequestHandler<UpdateGrowthCommand, GrowthMonitoringDto>
    {
        ...
        public UpdateGrowthRecordCommandHandler(
        ...
        public async Task<GrowthMonitoringDto> Handle(UpdateGrowthCommand request, CancellationToken cancellationToken)
        {
            var growth = await _growthMonitoringService.GetGrowthMonitoringById(request.Model.Id);
            if (growth != null)
            {
                 growth = request.Model.ToEntity(growth);
